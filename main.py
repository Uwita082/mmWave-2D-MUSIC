...
if __name__ == '__main__':
    ...
    # Per-channel (per array element) sample mean across time
    channel_means = np.mean(X, axis=1)  # shape (M,)
    sigma = np.std(X, axis=1)  # per-channel std
    se = sigma / np.sqrt(X.shape[1])  # expected fluctuation if mean=0
    ratio = np.abs(channel_means) / se  # ~O(1) means consistent with zero mean
    print("|mean| / standard_error:", ratio) # If ratio is roughly ≤ 2–3 for every channel, the data is statistically consistent with zero mean

    R_original = compute_covariance(X)  # Original covariance matrix

    print(np.shape(R_original))
